...
/**
 * One-time browser-triggerable seed endpoint - lets you populate demo data
 * without needing a local machine or CLI access. Visit this URL once after
 * your first deploy:
 *
 *   https://your-app.vercel.app/api/setup/seed?secret=YOUR_CRON_SECRET
 *
 * Reuses CRON_SECRET as the auth token so you don't need a separate env var.
 * Safe to call more than once (uses upserts / existence checks throughout).
 */
export async function GET(request: NextRequest) {
  ...
  for (const s of subjectData) {
    const id = s.nameEn.toLowerCase().replace(/\s+/g, "-");
    const created = await prisma.subject.upsert({ where: { id }, update: {}, create: { id, ...s } });
  ...
  for (const t of topicData) {
    ...
    if (!existing) {
      await prisma.topic.create({ data: { subjectId: subjects[t.subject], titleEn: t.titleEn, titleHi: t.titleHi, examPhase: t.phase } });
      topicsCreated++;
    }
  }
  ...
  const questionData = [
    { subject: "Bihar History", isPYQ: true, pyqYear: 2023, difficulty: "easy",
      ...
      correctOption: "A", explanationEn: "Chandragupta I founded the Gupta Empire around 320 CE.",
    ...
    { subject: "Indian Polity", isPYQ: true, pyqYear: 2024, difficulty: "medium",
      ...
      correctOption: "B", explanationEn: "There are 6 fundamental rights after the Right to Property was removed.",
    ...
    { subject: "Bihar Geography", isPYQ: true, pyqYear: 2022, difficulty: "medium",
      questionEn: 'Which river is known as the "Sorrow of Bihar"?', questionHi: 'किस नदी को "बिहार का शोक" कहा जाता है?',
      optionAEn: "Ganga", optionBEn: "Son", optionCEn: "Kosi", optionDEn: "Gandak",
      optionAHi: "गंगा", optionBHi: "सोन", optionCHi: "कोसी", optionDHi: "गंडक",
      correctOption: "C", explanationEn: "The Kosi river is called the Sorrow of Bihar due to frequent flooding.",
    ...
    { subject: "General Science", isPYQ: false, difficulty: "easy",
      ...
      correctOption: "B", explanationEn: "Force is measured in Newtons (N).",
    ...
    { subject: "Bihar History", isPYQ: true, pyqYear: 2025, difficulty: "easy",
      ...
      correctOption: "C", explanationEn: "Jharkhand was carved out of Bihar in November 2000.",
    ...
    { subject: "Indian Economy", isPYQ: true, pyqYear: 2024, difficulty: "hard",
      ...
      correctOption: "C", explanationEn: "The Fifth Five Year Plan (1974-79) emphasized growth with social justice.",
  ...
  for (const ca of currentAffairsData) {
    ...
    if (!exists) { await prisma.currentAffair.create({ data: ca }); caCreated++; }
  }
  ...
  await prisma.user.upsert({
    ...
    update: {},
    create: {
      name: "Demo Aspirant", email: "demo@bpscsaathi.com", passwordHash: demoPasswordHash,
      isVerified: true, onboardingCompleted: true, targetExamStage: "PRELIMS_MAINS",
      targetYear: 2027, prepLevel: "BEGINNER",
    },
  });

  return NextResponse.json({
    ok: true,
    topicsCreated, questionsCreated, currentAffairsCreated: caCreated,
    demoLogin: "demo@bpscsaathi.com / Demo@1234",
  });
}
